...
function renderThumbs(p) {
  ...
  thumbsEl.innerHTML = galleryImages.map((img, i) =>
    `<div class="modal-thumb ${i===galleryIdx?'active':''}" onclick="switchGalleryImg(${i})">
      ${img.src
        ? `<img src="${img.src}" alt="${p.name}" width="72" height="72" loading="lazy" onerror="this.parentElement.style.display='none'">`
  ...
  if (pdpGallery.length > 1) {
    thumbsEl.innerHTML = pdpGallery.map((url, i) =>
      `<div class="pdp-thumb ${i===pdpGalleryIdx?'active':''}" onclick="pdpGallerySet(${i})">
        <img src="${url}" alt="${escHtml(p.name)}" width="80" height="80" loading="lazy" onerror="this.style.display='none'">
      </div>`
    ).join('');
  } else {
    thumbsEl.innerHTML = '';
...
function _pdpSrchRender(q) {
  ...
  drop.innerHTML = _pdpSrchResults.map((p, i) => {
    ...
    const img = p.img
      ? `<img src="${escHtml(p.img)}" alt="${escHtml(p.name)}" class="pdp-drop-img" width="48" height="48" loading="lazy">`
