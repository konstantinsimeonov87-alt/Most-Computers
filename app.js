...
document.addEventListener('mc:productopen', e => {
  ...
  // Product price + availability for Facebook/WhatsApp rich previews
  ['product:price:amount', 'product:price:currency', 'product:availability'].forEach(prop => {
  ...
  const ppAmount = document.querySelector('meta[property="product:price:amount"]');
  if (ppAmount) ppAmount.setAttribute('content', (p.price / EUR_RATE).toFixed(2));
  const ppCurrency = document.querySelector('meta[property="product:price:currency"]');
  if (ppCurrency) ppCurrency.setAttribute('content', 'EUR');
  const ppAvail = document.querySelector('meta[property="product:availability"]');
  if (ppAvail) ppAvail.setAttribute('content', p.stock !== false ? 'instock' : 'oos');
});
...
document.addEventListener('DOMContentLoaded', function () { _whenProducts(initCatCounts); });
...
// Show skeleton while product data loads asynchronously
(function () {
  var grid = document.getElementById('productsGrid');
  if (grid && !grid.children.length) {
    grid.innerHTML = '<div style="padding:32px;text-align:center;color:var(--text-muted);grid-column:1/-1">Зарежда продукти…</div>';
  }
}());

// Quick cart badge from localStorage (runs immediately, no products needed)
...
_whenProducts(function () {
  ...
  // Restore old/pct/badge from the static snapshot (_staticProductsMap from data-loader.js).
  products.forEach(p => {
